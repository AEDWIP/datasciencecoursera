...
The next step to try a couple of different models. That is to say choose different covariants and see if we can improve the t values. I got the most signifigant results with predictors of wt and cyl. Adding gear, drat, and disp did not reduced signifigance so in the interested of having the simplest possible model I did not include them in my final model

Becuase am is our factor variable with the lowest level of the coveraients in our model, R will automatically select it as the reference variable. That is to say the 
expected value in mpg for cars with automatic transmissions. 
```{r}
